...
const LS_KEY_CSV='farol_csv_text_v1';const LS_KEY_TAB='farol_tab_v1';
...
function showTab(name){
  ...
  byId('tabbtn-'+name).classList.add('active');
  byId('tab-'+name).style.display='block';
}
...
function simular(){
  ...
  byId('simResult').innerHTML=`<div class="card"><b>TK global:</b> ${tk??'—'}<br>Plataforma ${plat} • NETID local ${net} • Canal ${chan}</div>`;
}
...
window.addEventListener('load',async()=>{
  ...
  document.getElementById('btnBuscar').addEventListener('click',searchTK);
  document.getElementById('btnSimular').addEventListener('click',simular);
  document.getElementById('platFilterAll').addEventListener('click',()=>renderList(null));
  document.getElementById('platFilter1').addEventListener('click',()=>renderList(1));
  document.getElementById('platFilter2').addEventListener('click',()=>renderList(2));
  document.getElementById('platFilter3').addEventListener('click',()=>renderList(3));
  document.getElementById('csvInput').addEventListener('change',handleCsvUpload);
  document.getElementById('btnExport').addEventListener('click',exportCsv);
  document.getElementById('btnReset').addEventListener('click',resetCsv);
  const ib=document.getElementById('installBtn'); if(ib) ib.addEventListener('click',doInstall);
});
